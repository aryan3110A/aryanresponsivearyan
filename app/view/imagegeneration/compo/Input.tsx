  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.image_url) {
        ...
        if (tokensDeducted) {
          setAvailableTokens(getTokens());
          setGeneratedImage(data.image_url); // ✅ Save for preview
      ...
      {showSelectionModel && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex">
          <div className="absolute left-0 top-0 h-full w-[380px]">
            <SelectionModel onClose={() => setShowSelectionModel(false)} onSave={handleSettingsSave} />
          </div>
        </div>
      )}

      {/* Input + Buttons */}
      <div className="flex items-center relative justify-center mb:flex-col mb:gap-4 w-full">
      ...
      {generatedImage && (
        <div className="mt-8 flex flex-col items-center">
          <img
            src={generatedImage}
            alt="Generated"
            className="w-[512px] h-auto mb:w-[90vw] rounded-xl border border-gray-700"
          />
          <button
            onClick={handleDownload}
            className="mt-4 bg-white/90 hover:bg-white text-black px-6 py-2 rounded-full font-medium"
          >
            Download Image
          </button>
        </div>
