      ...
      {isOpen && (
        <div className="space-y-4 mb-4 px-2 md:px-6">
          {options.map((option, index) => (
            <button
              key={index}
              className={`w-full p-3 rounded-lg text-left text-white text-sm md:text-lg transition-all duration-200 flex items-center justify-between
            ...
            >
              <span>{option}</span>
              <span className={`w-6 h-6 flex items-center justify-center rounded-full border-2 ${selectedOption === option ? "border-[#6C3BFF]" : "border-gray-500"}`}>
                {selectedOption === option && <span className="block w-3 h-3 rounded-full bg-[#6C3BFF]" />}
              </span>
            </button>
          ))}
        </div>
