...
export default function ProductWithModelPosePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch("https://748fae20dc68.ngrok-free.app/generate", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.image_urls) {
        // Multiple images - use image proxy to bypass ngrok warning
            setGeneratedImages(data.image_urls.map((url: string) => `/api/image-proxy?url=https://748fae20dc68.ngrok-free.app${url}`));
      } else if (data.image_url) {  
        // Single image (backward compatibility) - use image proxy to bypass ngrok warning
                  setGeneratedImages([`/api/image-proxy?url=https://748fae20dc68.ngrok-free.app${data.image_url}`]);
        } else {  
